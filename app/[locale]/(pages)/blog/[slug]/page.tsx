...
export default async function BlogPostPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-noir text-cream">
        ...
        <Container className="max-w-3xl space-y-8">
          <Reveal>
            <div className="space-y-5">
              ...
              <div className="flex items-center gap-4 text-sm text-inkMuted">
                <div className="flex items-center gap-3">
                  {/* Author avatar */}
                  <div className="flex h-10 w-10 items-center justify-center rounded-full border border-ink/15 bg-gradient-to-br from-espresso to-noir font-serif text-xs text-gold">
                    {post.author
                      .split(' ')
                      .map((n) => n[0])
                      .join('')}
                  ...
                  <div>
                    <p className="text-sm text-ink">{post.author}</p>
          ...
          {/* Featured hero image */}
          <Reveal delay={80}>
            <div className="relative aspect-[16/9] overflow-hidden rounded-2xl border border-ink/10 shadow-[0_35px_70px_rgba(0,0,0,0.15)]">
              {heroImage ? (
                <Image
                  src={heroImage}
                  alt={post.title}
          ...
          <Reveal delay={160}>
            <article className="mx-auto max-w-2xl">
                ...
                {post.content.map((paragraph, index) => (
                  <p
                    ...
                    className={`text-lg leading-[1.7] ${
                      index === 0
                        ? 'text-ink/90 first-letter:float-left first-letter:mr-2 first-letter:font-serif first-letter:text-5xl first-letter:leading-[0.8] first-letter:text-gold'
                        : 'text-inkMuted'
                    }`}
                  >
                    {paragraph}
              ...
              <blockquote className="my-10 border-l-2 border-gold/50 pl-6">
                <p className="font-serif text-xl italic leading-[1.6] text-ink/80 sm:text-2xl">
                  {t('detail.quote')}
                </p>
              ...
              <div className="mt-12 border-t border-ink/10 pt-8">
                <p className="mb-4 text-xs uppercase tracking-[0.3em] text-inkMuted">
                  {t('detail.share')}
                </p>
                <div className="flex gap-3">
                  ...
                  <a
                    ...
                    className="flex h-10 w-10 items-center justify-center rounded-full border border-ink/15 text-inkMuted transition hover:border-gold/50 hover:text-gold"
                  >
      ...
      {/* Related posts */}
      <Reveal>
        <section className="section-light border-t border-ink/10 py-16 sm:py-24">
          <Container className="space-y-10">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-2xl space-y-3">
                ...
                <p className="text-sm text-inkMuted sm:text-base">{t('related.description')}</p>
              </div>
              <Link
                href="/blog"
                ...
                <Reveal key={related.id} delay={index * 80} className="h-full">
                  ...
                  >
                    ...
                    <div className="mt-4 flex-1">
                      ...
                      <h3 className="mt-2 font-serif text-lg text-ink transition-colors duration-300 group-hover:text-gold">
                        {related.title}
                      </h3>
                      <p className="mt-2 text-sm text-inkMuted">{related.excerpt}</p>
                    </div>
                  </Link>
                </Reveal>
              ))}
