...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({ locale, page: 'checkout', path: 'checkout' });
}
...
export default async function CheckoutPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const checkoutBreadcrumb = breadcrumbSchema([
  ...
  return (
    <main className="min-h-screen bg-noir text-cream">
      <script
        ...
        dangerouslySetInnerHTML={{ __html: jsonLd(checkoutBreadcrumb) }}
      />
