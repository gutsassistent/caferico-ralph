...
2. Add a tiny idempotency helper (get/insert by paymentId) and wire `app/api/webhook/mollie/route.ts` to use it (remove the in‑memory Set). Depends on: 1. Test: hit webhook twice with same id → second call is a no‑op but still 200.
...
## Current
- Working on: Step 2 (idempotency helper + wire webhook)
- Iteration: 1
- Last action: Added molliePayments table to lib/schema.ts with paymentId (PK), status, processed, createdAt, updatedAt
